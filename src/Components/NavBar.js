...
function NavBar() {
  ...
  useEffect(()=>{
    ...
    const token =localStorage.getItem("access_token");
    const fetchDataByCategory =token && (async () => {
      try {
        const response = await fetch(url, {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <span className="navbar-toggler-icon"></span>
          ...
          {user.isConnected && <div className={isExpandNav?"collapse navbar-collapse show":"collapse navbar-collapse" } id="navbarSupportedContent">
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item dropdown" onClick={()=>setIsExpand(!isExpand)}>
                <a className={isExpand?"nav-link dropdown-toggle show":"nav-link dropdown-toggle"} 
                   href="#" id="navbarDropdown" role="button" data-toggle="dropdown"  aria-expanded={isExpand}>
                  Catalogue
                </a>
                <div className={isExpand?"dropdown-menu show rounded-0":"dropdown-menu"} aria-labelledby="navbarDropdown">
                  {category.map(cat=><a className="dropdown-item" href="#" onClick={()=>{setCategorySelected(cat.category)}}>{cat.category}</a>)}
                </div>
             </li>
            </ul>
            <div className="form-inline my-2 my-lg-0">
              <div className="col">
                ...
                >
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item dropdown" onClick={()=>setIsExpandPerson(!isExpandPerson)}>
                <a className={isExpandPerson?"nav-link dropdown-toggle show":"nav-link dropdown-toggle"} 
                   href="#" id="navbarDropdown" role="button" data-toggle="dropdown"  aria-expanded={isExpandPerson}>
                                    <Person size={20} />
                </a>
                <div className={isExpandPerson?"dropdown-menu show rounded-0":"dropdown-menu"} aria-labelledby="navbarDropdown">
                  <a className="dropdown-item" href="/order">Commandes</a>
                  <a className="dropdown-item" href="/wishlist">Liste de souhaits</a>
                  <a className="dropdown-item" href="/person-information">Information personnelle</a>
                  <a className="dropdown-item" href="/adress">Adresses</a>
                  <a className="dropdown-item" href="/payment-mode">Methodes de payements</a>
                </div>
             </li>
            </ul>
                </button>
